...
# from .MPNCOV.python import MPNCOV_no_inplace_ops as MPNCOV
import sys
...
class _NonLocalBlockND(nn.Module):
    ...
    def _embedded_gaussian(self, x):
        ...
        f = torch.matmul(theta_x, phi_x)
        # return f
        f_div_C = F.softmax(f, dim=-1) # TODO ORIGINAL

        # f_log_softmax = F.log_softmax(f, dim=-1)  # TODO MODIFIED
        # f_div_C = torch.exp(f_log_softmax)  # TODO MODIFIED
...
class CALayer(nn.Module):
    def __init__(self, channel, reduction=8):
        ...
        # feature channel downscale and upscale --> channel weight # TODO
...
class LSRAG(nn.Module):
    ...
    def forward(self, x):
        ...
        for i,l in enumerate(self.rcab):
            ...
            x = l(x)
            # TODO normalize the x residual to avoid gradient explosion !!!
            # x = x / (torch.norm(x, keepdim=True) + TINY)
...
class SAN(nn.Module):
    def __init__(self, conv=common.default_conv, scale=2, channels=1, **kwargs):
    ...
    def forward(self, x):
        ...
        return x
        # return x, res  # TODO - Embedding Extraction
